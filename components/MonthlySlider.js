...
function BoundaryLayer({ data }) {
    const { useMap, GeoJSON } = require("react-leaflet");
    const map = useMap();
    useEffect(() => {
        if (data) {
...
function MonthlySlider() {
    ...
    const fetchRangeSeries = (lat, lon, fy, fm, ty, tm, isSecond = false) => {
        ...
        const url = (y, m) => `${process.env.NEXT_PUBLIC_BASE_URL}/ndvi/wcma_sample?year=${y}&month=${m}&lat=${lat}&lon=${lon}`
    ...
    const fetchNdvi = useCallback((monthsBack) => {
        ...
        fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/ndvi/wcma_monthly?year=${year}&month=${month}`)
            .then(res => res.json())
            .then(data => {
    ...
    useEffect(() => {
        ...
        const checkMonth = (yy, mm) => {
            return fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/ndvi/count_images?year=${yy}&month=${mm}`)
                .then(r => r.json())
                .then(d => {
                    if (d.count >= 100) {
                    ...
                    <MapContainer style={{ height: "90vh" }}>
                        ...
                        <ClickHandler
                            onClick={(latlng) => {
                                ...
                                setLabel(label)
                                fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/ndvi/wcma_sample?year=${year}&month=${month}&lat=${latlng.lat}&lon=${latlng.lng}`)
                                    .then(res => res.json())
                                    .then(data => {
                                        if (data.inside) {
